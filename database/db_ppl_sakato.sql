...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 14, 2022 at 10:34 PM
...
INSERT INTO `pengiriman` (`kd_pengiriman`, `kd_transaksi`, `status_pengiriman`, `ekspedisi`, `ekspedisi_service`, `ongkir`, `no_resi`) VALUES
(12, 21, 4, 'pos', 'Pos Reguler', 57500, 'JNT002');
...
INSERT INTO `produk` (`id`, `nama`, `harga`, `foto_produk`, `keterangan`, `berat`) VALUES
(1, 'Seragam Putih Dongker', 120000, '7759c2cff3a229915129b09b1afaaa8e.jpeg', 'deskripsi keterangan produk', 250),
(2, 'Seragam Batik', 100000, '78b31efe49eff8bfa3da23b4dd3db991.jpeg', 'deskripsi keterangan produk', 250),
(3, 'Seragam Muslim', 100000, 'e43d85da9e4c6cc263a82766275edc9c.jpeg', 'deskripsi keterangan produk', 250),
(4, 'Seragam Pramuka', 120000, '5ae9b080d984682f31ac55b5ec2b2061.jpeg', 'deskripsi keterangan produk', 250),
(5, 'Seragam Olah Raga', 110000, '924ba36992a191c44746fce665b0fa85.jpeg', 'deskripsi keterangan produk', 250);
...
INSERT INTO `transaksi` (`kd_transaksi`, `total_harga`, `bukti_pembayaran`, `status_pembayaran`, `tgl`, `status_transaksi`, `id_customer`, `total_biaya`, `id_user_toko`) VALUES
(21, 100000, '3d7575ace11fab4f86097d8d6496324e.jpg', 4, '2022-09-14', 5, 7, 157500, 2);
...
(7, 'smpku', 'smpku', 'smpku', 'Sanda', '0809879', '14', '136', '80351'),
...
(12, 'smpku', NULL, 'smpku', 'Sanda', '0809879', '4', '64', '80351'),
(13, 'smpku', NULL, 'smpku', 'Sanda', '0809879', '22', '239', '80351');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_pengiriman`  AS SELECT `v_transaksi`.`kd_transaksi` AS `kd_transaksi`, `v_transaksi`.`total_harga` AS `total_harga`, `v_transaksi`.`bukti_pembayaran` AS `bukti_pembayaran`, `v_transaksi`.`status_pembayaran` AS `status_pembayaran`, `v_transaksi`.`tgl` AS `tgl`, `v_transaksi`.`status_transaksi` AS `status_transaksi`, `v_transaksi`.`id_customer` AS `id_customer`, `v_transaksi`.`total_biaya` AS `total_biaya`, `v_transaksi`.`id_user_toko` AS `id_user_toko`, `v_transaksi`.`id` AS `id`, `v_transaksi`.`nama` AS `nama`, `v_transaksi`.`username` AS `username`, `v_transaksi`.`alamat` AS `alamat`, `v_transaksi`.`nama_penanggung_jawab` AS `nama_penanggung_jawab`, `v_transaksi`.`telp` AS `telp`, `v_transaksi`.`provinsi` AS `provinsi`, `v_transaksi`.`kota` AS `kota`, `v_transaksi`.`kode_pos` AS `kode_pos`, `pengiriman`.`kd_pengiriman` AS `kd_pengiriman`, `pengiriman`.`status_pengiriman` AS `status_pengiriman`, `pengiriman`.`ekspedisi` AS `ekspedisi`, `pengiriman`.`ekspedisi_service` AS `ekspedisi_service`, `pengiriman`.`ongkir` AS `ongkir`, `pengiriman`.`no_resi` AS `no_resi` FROM (`pengiriman` join `v_transaksi`) WHERE `pengiriman`.`kd_transaksi` = `v_transaksi`.`kd_transaksi``kd_transaksi`  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_pesanan`  AS SELECT `pemesanan`.`kd_pemesanan` AS `kd_pemesanan`, `pemesanan`.`id_customer` AS `id_customer`, `pemesanan`.`kd_produk` AS `kd_produk`, `pemesanan`.`kd_transaksi` AS `kd_transaksi`, `pemesanan`.`jumlah` AS `jumlah`, `pemesanan`.`sub_total` AS `sub_total`, `pemesanan`.`size` AS `size`, `produk`.`id` AS `id`, `produk`.`nama` AS `nama`, `produk`.`harga` AS `harga`, `produk`.`foto_produk` AS `foto_produk`, `produk`.`keterangan` AS `keterangan`, `produk`.`berat` AS `berat` FROM (`pemesanan` join `produk`) WHERE `produk`.`id` = `pemesanan`.`kd_produk``kd_produk`  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_transaksi`  AS SELECT `transaksi`.`kd_transaksi` AS `kd_transaksi`, `transaksi`.`total_harga` AS `total_harga`, `transaksi`.`bukti_pembayaran` AS `bukti_pembayaran`, `transaksi`.`status_pembayaran` AS `status_pembayaran`, `transaksi`.`tgl` AS `tgl`, `transaksi`.`status_transaksi` AS `status_transaksi`, `transaksi`.`id_customer` AS `id_customer`, `transaksi`.`total_biaya` AS `total_biaya`, `transaksi`.`id_user_toko` AS `id_user_toko`, `user_customer`.`id` AS `id`, `user_customer`.`nama` AS `nama`, `user_customer`.`username` AS `username`, `user_customer`.`alamat` AS `alamat`, `user_customer`.`nama_penanggung_jawab` AS `nama_penanggung_jawab`, `user_customer`.`telp` AS `telp`, `user_customer`.`provinsi` AS `provinsi`, `user_customer`.`kota` AS `kota`, `user_customer`.`kode_pos` AS `kode_pos` FROM (`transaksi` join `user_customer`) WHERE `transaksi`.`id_customer` = `user_customer`.`id``id`  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_user_customer`  AS SELECT `user_customer`.`id` AS `id`, `user_customer`.`nama` AS `nama`, `user_customer`.`username` AS `username`, `user_customer`.`alamat` AS `alamat`, `user_customer`.`nama_penanggung_jawab` AS `nama_penanggung_jawab`, `user_customer`.`telp` AS `telp`, `user_customer`.`provinsi` AS `provinsi`, `user_customer`.`kota` AS `kota`, `user_customer`.`kode_pos` AS `kode_pos`, `user`.`password` AS `password`, `user`.`level` AS `level` FROM (`user` join `user_customer`) WHERE `user`.`username` = `user_customer`.`username``username`  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_user_toko`  AS SELECT `user_toko`.`id` AS `id`, `user_toko`.`nama` AS `nama`, `user_toko`.`username` AS `username`, `user_toko`.`jabatan` AS `jabatan`, `user`.`password` AS `password`, `user`.`level` AS `level` FROM (`user` join `user_toko`) WHERE `user`.`username` = `user_toko`.`username``username`  ;
...
--
ALTER TABLE `pemesanan`
  MODIFY `kd_pemesanan` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=33;

--
-- AUTO_INCREMENT for table `pengiriman`
--
ALTER TABLE `pengiriman`
  MODIFY `kd_pengiriman` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
...
--
ALTER TABLE `transaksi`
  MODIFY `kd_transaksi` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=22;
...
--
ALTER TABLE `pengiriman`
  ADD CONSTRAINT `pengiriman_ibfk_1` FOREIGN KEY (`kd_transaksi`) REFERENCES `transaksi` (`kd_transaksi`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `transaksi`
  ...
  ADD CONSTRAINT `transaksi_ibfk_2` FOREIGN KEY (`id_user_toko`) REFERENCES `user_toko` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
